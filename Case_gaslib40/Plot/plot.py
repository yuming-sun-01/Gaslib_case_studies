"""
plot.py — E-NMPC comparison plot for the Gaslib40 network using local Excel data.
...
Layout (2 rows x N columns):
    Row 1 (top):    Source flow on LEFT y-axis (kg/s) and source pressure on
                    RIGHT y-axis.
    Row 2 (bottom): Compressor energies (kWh) with OCSS dotted.

Columns:
    All xlsx files in this folder except css.xlsx and generated summaries.

OCSS reference comes from `css.xlsx`, tiled across the visible time axis.
...
# Truncate x-axis to the shortest available scenario by default. Set an integer
# here to force a specific horizon.
X_MAX_HOURS = None

# Files generated by this script or used only as reference.
EXCLUDE_XLSX = {"css.xlsx", "obj.xlsx"}

# OCSS reference: Gaslib40 uses a 24-hour periodic cycle.
OCSS_FILE          = "css.xlsx"
OCSS_CYCLE_HOURS   = 24
OCSS_DROP_LAST     = 1     # drop the duplicated periodic endpoint

# Gaslib40 source display. The script filters these against columns actually
# present in css.xlsx.
...
def scenario_title(filename):
    stem = Path(filename).stem
    if stem.startswith("f_enmpc_cycle_"):
        try:
            n = int(stem.rsplit("_", 1)[-1])
        except ValueError:
            n = None
        if n is not None:
            unit = "cycle" if n == 1 else "cycles"
            return f"Finite horizon E-NMPC\nHorizon – {n} {unit}"
    if stem.startswith("Inf_enmpc"):
        return "Infinite horizon E-NMPC\nFinite segment – 1 cycle"
...
COLUMNS = [
    (p.name, scenario_title(p.name), None)
    for p in sorted(
...
def load_cpu_time_seconds(path, max_steps=None):
    """Read total CPU time in seconds from the NMPC Excel file by summing iteration times.
    When max_steps is provided, sums only the first max_steps iteration rows so
    CPU time uses the same truncated horizon as the plotted trajectories.
    """
