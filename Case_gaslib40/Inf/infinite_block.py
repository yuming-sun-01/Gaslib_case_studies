...
def _interp_24h(df, col, phase):
    phase = float(phase) % 24.0
    df_24 = df.loc[[x for x in df.index if 0.0 <= float(x) <= 24.0]]
    xs = np.asarray([float(x) for x in df_24.index], dtype=float)
    ys = np.asarray(df_24[col], dtype=float)
    order = np.argsort(xs)
    return float(np.interp(phase, xs[order], ys[order]))
    ...
    for idx in ib.WConsIndex:
        ...
        for tau in time_points:
            if col is None:
                ...
                val = _interp_24h(wcons_df, col, phase)
                n_css += 1
...
def _init_infinite_interm_p_css(ib, m_dyn, ocss_file_path,
    ...
    for (p, vol) in m_dyn.Pipes_VolExtrR_interm:
        ...
        for tau in time_points:
            ...
            ib.interm_p_css[p, vol, tau] = _interp_24h(df, col, phase)


def _init_infinite_pSource_css(ib, m_dyn, ocss_file_path,
                               T_finite, L, T_period, time_points):
    try:
        df = pd.read_excel(ocss_file_path, sheet_name="pSource", index_col=0)
    except Exception as err:
        print(f"[inf-block] WARNING: could not read CSS pSource sheet ({err})")
        return

    col_by_norm = {_norm(c): c for c in df.columns}
    for s in ib.lyapunov_pSource_index:
        col = col_by_norm.get(_norm(f"pSource[{s},:]"))
        if col is None:
            col = next((c for c in df.columns if _norm(str(s)) in _norm(c)), None)
        if col is None:
            continue
        for tau in time_points:
            phase = 0.0 if _is_unit_time(float(tau)) else \
                (T_finite + L * math.atanh(float(tau))) % T_period
            ib.pSource_css[s, tau] = _interp_24h(df, col, phase)
...
def _prune_inf_block_to_collocation_points(m_dyn):
    ...
    keep_names = {'pipe_rho', 'interm_p', 'pSource', 'phi_P', 'phi_V'}
    ...
    print(f"[inf-block] collocation pruning (CSTR-style): -{n_var} var entries, "
          f"-{n_con} constraint entries (pipe_rho, interm_p, pSource & phi_P kept at all times; "
...
def _add_infinite_block(m_dyn, ocss_file_path, T_period=24.0,
    ...
    if add_lyapunov_phi_V:
        tracked_psources = [
            s for s in m_dyn.NodesSources
            if str(s) in TRACKED_PSOURCE_NAMES
        ]
        ib.lyapunov_pSource_index = pyo.Set(initialize=tracked_psources)
        ib.interm_p_css = pyo.Param(
            m_dyn.Pipes_VolExtrR_interm, ib.time, mutable=True, initialize=55.0,
        )
        ib.pSource_css = pyo.Param(
            ib.lyapunov_pSource_index, ib.time,
            mutable=True, initialize=55.0,
        )
        _init_infinite_interm_p_css(
            ib, m_dyn, ocss_file_path, T_finite, L, T_period, time_points,
        )
        _init_infinite_pSource_css(
            ib, m_dyn, ocss_file_path, T_finite, L, T_period, time_points,
        )

        # De-singularized phi_V ODE: drop the (1 - tau^2) tail-transform factor,
        # which vanishes as tau->1 and makes this row ill-conditioned (tiny coeff
        # on dphi_V_dt vs O(1) on interm_p). With it removed, dphi_V_dt stays
        # finite at the last collocation point and the row is well scaled.
        # NOTE: this changes phi_V's meaning from the exact real-time integral to
        # a bounded tau-uniform integral of the tracking error (gamma kept only
        # as a constant scale); it is a Lyapunov surrogate, not the transformed
        # cost-to-go.

        # This objective will be recalculate in objective.py
        def _phi_V_ode_rule(b, t):
            ...
            stage += sum(
                (b.pSource[s, t] - b.pSource_css[s, t]) ** 2
                for s in b.lyapunov_pSource_index
            )
            return b.gamma * b.dphi_V_dt[t] == stage

        ib.phi_V_ode = pyo.Constraint(ib.time, rule=_phi_V_ode_rule)
        ib.phi_V[0].fix(0.0)
        print("[inf-block] added Lyapunov integral state phi_V "
              "(interm_p+pSource3 tracking)")

    tf = m_dyn.Times.last()
    t0 = ib.time.first()
    # Match the finite control structure on the infinite tail: pressure sources
    # (source_1, source_2) pin pSource; flow source (source_3) pins wSource.
    # Values are taken from the finite-block boundary (tf), like the rest of the
    # tail pinning.
    wsource_fixed_nodes = {'source_3'}
    n_ps_fixed = n_ws_fixed = 0
    for s in m_dyn.NodesSources:
        if str(s) in wsource_fixed_nodes:
            val = pyo.value(m_dyn.wSource[s, tf])
            for t in time_points:
                ib.wSource[s, t].set_value(val)
                ib.wSource[s, t].fix()
                n_ws_fixed += 1
        else:
    ...
    print(f"[inf-block] pSource fixed on infinite tail at {n_ps_fixed} point(s), "
          f"wSource fixed at {n_ws_fixed} point(s).")
    ...
    terminal_pSource_index = [
        s for s in m_dyn.NodesSources
        if str(s) in TRACKED_PSOURCE_NAMES
    ]
    css_pSource0 = _load_css_zero(
        ocss_file_path, "pSource", "pSource", terminal_pSource_index,
    )
    css_pSource0 = {
        k[0] if isinstance(k, tuple) and len(k) == 1 else k: v
        for k, v in css_pSource0.items()
    }
    print(f"[inf-block] Terminal CSS hard constraint at tau=1: "
          f"{len(css_rho0) if terminal_css_constraint else 0} interm_p "
          f"and {len(css_pSource0) if terminal_css_constraint else 0} pSource entries "
          f"compared to CSS t=0")
    ...
    m_dyn._phase_info = {
        ...
        'terminal_css_pSource0': css_pSource0 if terminal_css_constraint else {},
    ...
    # Build the Lyapunov integral state phi_V inert first so the caller can
    # choose when it enters the NLP. controller_factory.run_model activates it
    # before the init combined solve, giving that solve and MPC step 0 the same
    # phi_V structure while keeping construction/pruning simple here.
    if add_lyapunov_phi_V:
        set_lyapunov_phi_V_active(m_dyn, active=False)
        print("[inf-block] phi_V built INERT (fixed + deactivated); "
              "caller activates it before the init combined solve")

    return m_dyn


def set_lyapunov_phi_V_active(m_dyn, active):
    """Toggle the Lyapunov integral-state (phi_V) subsystem on/off.

    Inert (active=False): phi_V and dphi_V_dt are fixed to 0 and every phi_V
    constraint (the ODE + its collocation discretization/continuity equations)
    is deactivated, so the subsystem contributes nothing to the solve. Active
    (active=True): phi_V (except phi_V[0]=0) and dphi_V_dt are unfixed and all
    phi_V constraints are reactivated. The current controller build activates
    phi_V before the init combined solve so step 0 has no structural toggle.
    """
    ib = getattr(m_dyn, 'inf_block', None)
    if ib is None or not hasattr(ib, 'phi_V'):
        return
    t0 = ib.time.first()
    for idx in ib.phi_V:
        if idx == t0:
            continue  # phi_V[0] is always pinned to 0
        ib.phi_V[idx].unfix() if active else ib.phi_V[idx].fix(0.0)
    if hasattr(ib, 'dphi_V_dt'):
        for idx in ib.dphi_V_dt:
            ib.dphi_V_dt[idx].unfix() if active else ib.dphi_V_dt[idx].fix(0.0)
    for con in ib.component_objects(pyo.Constraint, descend_into=False):
        if 'phi_V' in con.name.split('.')[-1]:
            con.activate() if active else con.deactivate()
...
def _add_hard_terminal_constraint(m_dyn, terminal_error_bound=1.0e-6):
    ...
    css_pSource0 = phase_info.get('terminal_css_pSource0', {})

    if not css_rho0 and not css_pSource0:
        print("[constraint] WARNING: no terminal CSS references found, skipping hard constraint")
        return m_dyn
    ...
    ib.terminal_pSource_index = pyo.Set(
        initialize=list(css_pSource0.keys()),
    )
    ib.terminal_pSource_target = pyo.Param(
        ib.terminal_pSource_index, mutable=True,
        initialize={k: css_pSource0[k] for k in css_pSource0},
    )
    ...
    terminal_error += sum(
        (ib.pSource[s, t_last] - ib.terminal_pSource_target[s]) ** 2
        for s in ib.terminal_pSource_index
    )
    ...
    print(f"[constraint] Added aggregated terminal constraint: "
          f"sum terminal tracking errors <= {terminal_error_bound:g} "
          f"over {len(css_rho0)} interm_p and {len(css_pSource0)} pSource entries")
