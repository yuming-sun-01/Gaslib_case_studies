...
def _norm(s):
    """Normalize label: remove quotes/spaces (for matching CSS column names)."""
...
def dynamic_demand_profile(ss_demand, num_time_periods = 1, epsilon = 0):
    # Sinusoidal demand with a TRUE period of T_PERIOD hours, matching the
    # gaslib40 CSS reference (built by Finite/run_model_css_72.py, which uses
    # ss + ss*sin(2*pi*k/24)*amplitude with amplitude = 1/20). Making the period
    # intrinsic (independent of array length / num_time_periods) keeps both the
    # finite warm-start demand and the rolling MPC demand consistent with CSS.
    T_PERIOD = 24.0
    amplitude = 1 / 20
    time_length = len(ss_demand)
    ss_demand = np.asarray(ss_demand)

    x = 2 * np.pi * np.arange(time_length) / T_PERIOD
    dynamic_demand = ss_demand + ss_demand * np.sin(x) * amplitude \
        + epsilon * np.sin(x) * amplitude
