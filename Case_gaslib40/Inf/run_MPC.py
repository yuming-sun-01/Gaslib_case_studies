...
from infinite_block import set_lyapunov_phi_V_active
...
def _interp_24h(df, col, phase):
    """Linear interpolation on one 24 h CSS period."""
...
def _build_demand_data(m_steady, simulation_steps, opts):
    ...
    """
    T_period_steps = int(round(opts.T_period / opts.sample_time))  # e.g. 24
...
def _refresh_phase_references(m_controller, sim_t0):
    ...
    def _val(df, col, phase):
        if df is None or col is None:
            return None
        try:
            return _interp_24h(df, col, phase)
        except Exception:
            return None
...
def _refresh_terminal_css_reference(m_controller, sim_t0):
    ...
    for (p, vol) in ib.terminal_css_index:
        ...
        if col is None:
            continue
        ib.terminal_css_target[p, vol] = _interp_24h(df, col, phase)
        n += 1

    n_ps = 0
    if hasattr(ib, 'terminal_pSource_target'):
        df_ps = _css_df(ocss_path, 'pSource')
        if df_ps is not None:
            ps_col_by_norm = {_norm(str(c)): c for c in df_ps.columns}
            for s in ib.terminal_pSource_index:
                col = ps_col_by_norm.get(_norm(f"pSource[{s},:]"))
                if col is None:
                    col = next((c for c in df_ps.columns
                                if _norm(str(s)) in _norm(c)), None)
                if col is None:
                    continue
                ib.terminal_pSource_target[s] = _interp_24h(df_ps, col, phase)
                n_ps += 1
    print(f"  [endpoint] rolled CSS reference to phase {phase:.2f} h "
          f"({n} interm_p, {n_ps} pSource targets)")
    return phase
...
def _refresh_lyap_css_reference(m_controller, sim_t0):
    ...
    if df is None:
        return
    df_ps = _css_df(str(info['ocss_file_path']), 'pSource')
    col_by_norm = {_norm(str(c)): c for c in df.columns}
    col_cache = {}
    ...
    n_ps = 0
    if df_ps is not None and hasattr(m_controller, 'pSource_css_fin'):
        ps_col_by_norm = {_norm(str(c)): c for c in df_ps.columns}
        ...
        for s in m_controller.lyapunov_pSource_index:
            col = _ps_col(s)
            if col is None:
                continue
            for t in m_controller.Times:
                if t == last_t:
                    continue
                m_controller.pSource_css_fin[s, t] = _interp_24h(
                    df_ps, col, (float(sim_t0) + float(t)) % T_period)
                n_ps += 1
            if ib is not None and hasattr(ib, 'pSource_css'):
                for tau in sorted(ib.time):
                    if float(tau) == 1.0:
                        phase = float(sim_t0) % T_period
                    else:
                        phase = (float(sim_t0) + T_finite
                                 + L * math.atanh(min(float(tau), tau_max))) % T_period
                    ib.pSource_css[s, tau] = _interp_24h(df_ps, col, phase)
                    n_ps += 1
    print(f"  [lyapunov] rolled interm_p+pSource CSS reference "
          f"to sim_t0={sim_t0:.2f} h ({n_ps} pSource updates)")


def _plant_lyap_stage_cost(m_controller, m_plant, phase):
    """Realized plant tracking cost at plant last time."""
    ocss_path = str(m_controller._phase_info['ocss_file_path'])
    df = _interm_p_css_df(ocss_path)
    if df is None:
        return 0.0
    df_ps = _css_df(ocss_path, 'pSource')
    col_by_norm = {_norm(str(c)): c for c in df.columns}
    ...
    for (p, vol) in m_controller.Pipes_VolExtrR_interm:
        ...
        if col is None:
            continue
        e = pyo.value(m_plant.interm_p[p, vol, t_last_p]) - _interp_24h(df, col, phase)
        sse += e * e
    if df_ps is not None and hasattr(m_controller, 'lyapunov_pSource_index'):
        ps_col_by_norm = {_norm(str(c)): c for c in df_ps.columns}
        for s in m_controller.lyapunov_pSource_index:
            col = ps_col_by_norm.get(_norm(f"pSource[{s},:]"))
            if col is None:
            ...
            if col is None:
                continue
            e = (pyo.value(m_plant.pSource[s, t_last_p])
                 - _interp_24h(df_ps, col, phase))
            sse += e * e
    return sse


def _report_endpoint_error(m_controller):
    """Print terminal-constraint LHS error."""
    ib = getattr(m_controller, 'inf_block', None)
    if ib is None or not hasattr(ib, 'terminal_css_target'):
        return
    ...
    if hasattr(ib, 'terminal_pSource_target'):
        for s in ib.terminal_pSource_index:
            e = (pyo.value(ib.pSource[s, t_last])
                 - pyo.value(ib.terminal_pSource_target[s]))
            sse += e * e
            max_err = max(max_err, abs(e))
            n_ps += 1
    ...
    if viol <= 1e-4:
        ...
        status = "VIOLATED"
    print(f"  [endpoint] LHS terminal CSS error = {sse:.6e} "
          f"(bound {bound:.2e}, {status}); max|err| = {max_err:.6e}; "
          f"pSource targets={n_ps}")
...
def _build_plant(controller_t0_inputs, m_steady, opts):
    ...
    # Source controls from steady-state: source_1 / source_2 fix pSource,
    # flow source source_3 fixes wSource (same as run_model_css_72.py).
...
def run_nmpc(simulation_steps=None, sample_time=None, opts=None):
    ...
    if _stability_on:
        apply_stability_constraint(
            m_controller,
            delta=float(getattr(opts, 'delta', 0.05)),
            lyapunov_inf_scale=float(getattr(opts, 'lyapunov_inf_scale', 1.0)),
            delta_percent=float(getattr(opts, 'delta_percent', 0.0)),
            delta_finite=float(getattr(opts, 'delta_finite', 1.0)),
        )
    step_change = int(getattr(opts, 'step_change', simulation_steps + 1))
    ...
    for i in range(simulation_steps):
        ...
        if _stability_on:
            # phi_V is built INERT (kept out of the warm-up solve); turn the
            # Lyapunov integral state on here, inside the iteration, once.
            if i == 0:
                set_lyapunov_phi_V_active(m_controller, active=True)
            _refresh_lyap_css_reference(m_controller, sim_t0=sim_t0)
            use_first_stage_cost = 1.0 if i >= step_change else 0.0
            m_controller.use_first_stage_cost_descent.set_value(use_first_stage_cost)
            if i == 0:
    ...
    # ------- Lyapunov: print V every step + update prev params -------
        ...
        # the next step. This follows the finite-horizon form:
        ...
        if _stability_on:
            ...
            # Split V = scaled finite horizon part + scaled infinite tail.
            _ib = m_controller.inf_block
            V_inf_raw = pyo.value(_ib.phi_V[sorted(_ib.time)[-1]])
            V_inf_scale = pyo.value(m_controller.lyapunov_inf_scale)
            V_inf = V_inf_raw / V_inf_scale
            V_fin_scale = pyo.value(m_controller.delta_finite)
            V_fin_scaled = V_cur - V_inf
            V_fin_raw = V_fin_scaled * V_fin_scale
            con_state = ("inactive" if not m_controller.stability_constraint.active
                         else "ACTIVE")
            descent_mode = ("first-stage-cost"
                            if pyo.value(m_controller.use_first_stage_cost_descent) >= 0.5
                            else "percent")
            print(f"  [lyapunov] step {i} (t={sim_t0:.1f}h): V_current={V_cur:.6e} "
                  f"(finite={V_fin_scaled:.6e} "
                  f"[raw finite={V_fin_raw:.6e}, scale={V_fin_scale:.6e}], "
                  f"inf={V_inf:.6e} "
                  f"[raw phi_V={V_inf_raw:.6e}, scale={V_inf_scale:.6e}]) "
                  f"(stability constraint {con_state}, mode={descent_mode})")
            if _solve_ok:
                phase_plant = (sim_t0 + sample_time) % T_period
                ell_plant_raw = _plant_lyap_stage_cost(
                    m_controller, m_plant, phase_plant)
                ell_plant_scaled = ell_plant_raw / V_fin_scale
                next_uses_first_stage_cost = (i + 1) >= step_change
                if next_uses_first_stage_cost:
                    V_prev_next = V_cur
                    requirement = "first-stage-cost descent"
                    bound = (V_prev_next
                             - pyo.value(m_controller.delta) * ell_plant_scaled)
                else:
                    V_prev_next = V_cur
                    requirement = "percent descent"
                    bound = ((1.0 - pyo.value(m_controller.delta_percent))
                             * V_prev_next)
                m_controller.lyapunov_function_prev.set_value(V_prev_next)
                m_controller.tracking_cost_plant_prev.set_value(ell_plant_scaled)
                print(f"  [lyapunov] plant tracking stage cost={ell_plant_scaled:.6e} "
                      f"[raw={ell_plant_raw:.6e}, scale={V_fin_scale:.6e}]; "
                      f"next-step {requirement} uses V_prev={V_prev_next:.6e} "
                      f"with plant_delta={pyo.value(m_controller.delta):.6e} "
                      f"and requires V <= {bound:.6e}")
    ...
    with open("hierarchical_timer_mpc_inf_gaslib40.txt", "w") as f:
        f.write(str(timer))
...
if __name__ == "__main__":
    ...
    OUTPUT_PATH = str(Path(opts.output_path).with_name("0.1_0_1_1_gamma10_no_warm.xlsx"))
    LOG_PATH    = str(Path(opts.output_path).with_name("0.1_0_1_1_gamma10_no_warm.txt"))
    _tee = _Tee(LOG_PATH)
