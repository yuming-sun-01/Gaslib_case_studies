...
if __name__ == "__main__":
    # ---- Run CSS calculation ------------------------------------------ #
    ocss_output_path = os.path.join(_BASE_DIR, "optimal_css_24hrs_gaslib40.xlsx")
    ...
    # ---- Append CPU time + objective to xlsx --------------------------- #
    summary_df = pd.DataFrame([{
        'steady_cpu_s':  timing['steady_cpu_s'],
        'dynamic_cpu_s': timing['dynamic_cpu_s'],
        'total_cpu_s':   timing['total_cpu_s'],
        'objective':     pyo.value(m_dyn.ObjFun),
    }])
    with pd.ExcelWriter(ocss_output_path, engine='openpyxl', mode='a',
                        if_sheet_exists='replace') as writer:
        summary_df.to_excel(writer, sheet_name='summary', index=False)
    print(f"[timing] written to '{ocss_output_path}' sheet 'summary'")
