...
from run_model_css_72_soft import run_model
from utils import import_data_from_excel
import json
from stability import apply_stability_constraint
from terminal import refresh_css_references_for_horizon
...
def load_wcons_demand_from_excel(excel_path, sink_node_set, min_length):
    df = pd.read_excel(excel_path, sheet_name="wCons", index_col=0)
    demand_data = {}

    for s in sink_node_set:
        col = f"wCons['{s}', 0, :]"
        if col not in df.columns:
            raise KeyError(f"Cannot find demand column {col} in {excel_path}")

        values = list(df[col].astype(float))
        if len(values) > 1 and abs(values[0] - values[-1]) <= 1e-9:
            values = values[:-1]
        if len(values) < min_length:
            n_repeat = (min_length + len(values) - 1) // len(values)
            values = (values * n_repeat)[:min_length]
        demand_data[s] = values
...
def run_nmpc(
    ...
    min_demand_length = simulation_steps * sample_time + controller_horizon + 1
    demand_data = load_wcons_demand_from_excel(
        ocss_file_path, sink_node_set, min_demand_length)
    ...
    for i in range(simulation_steps):
        ...
        refresh_css_references_for_horizon(m_controller, sim_t0=sim_t0)
        terminal_phase = sim_t0 + controller_horizon
        css_period = getattr(m_controller, "_css_period", controller_horizon)
        print(f"[terminal] target phase={terminal_phase} -> CSS time {terminal_phase % css_period}")
...
if __name__ =="__main__":
    ...
    write_data_to_excel(sim_data, m_plant, sheets_keys_dict, "final_paper_enmpc_explicit_terminal_each_point_stability_72hrs_soft.xlsx",
