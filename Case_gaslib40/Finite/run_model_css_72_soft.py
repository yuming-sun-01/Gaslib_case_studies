...
from terminal import css_terminal_constraints_final_to_ocss_phase0
import json
import os
import time
import pandas as pd
...
def _apply_terminal_l1_soft_penalty(m, penalty=100.0):
    tf = m.Times.last()

    if hasattr(m, "terminal_pressure_phase0"):
        m.terminal_pressure_phase0.deactivate()
    if hasattr(m, "terminal_controls_phase0"):
        m.terminal_controls_phase0.deactivate()

    m.terminal_pressure_abs = pyo.Var(
        m.Pipes_VolExtrR_interm, within=pyo.NonNegativeReals, initialize=0.0)
    m.terminal_compressor_abs = pyo.Var(
        m.Stations, within=pyo.NonNegativeReals, initialize=0.0)

    def _terminal_pressure_pos(m, p, vol):
        return (
            m.interm_p[p, vol, tf] - m.terminal_interm_p_target[p, vol]
            <= m.terminal_pressure_abs[p, vol])
    m.terminal_pressure_l1_pos = pyo.Constraint(
        m.Pipes_VolExtrR_interm, rule=_terminal_pressure_pos)

    def _terminal_pressure_neg(m, p, vol):
        return (
            m.terminal_interm_p_target[p, vol] - m.interm_p[p, vol, tf]
            <= m.terminal_pressure_abs[p, vol])
    m.terminal_pressure_l1_neg = pyo.Constraint(
        m.Pipes_VolExtrR_interm, rule=_terminal_pressure_neg)

    def _terminal_compressor_pos(m, c):
        return (
            m.compressor_P[c, tf] - m.terminal_compressor_P_target[c]
            <= m.terminal_compressor_abs[c])
    m.terminal_compressor_l1_pos = pyo.Constraint(
        m.Stations, rule=_terminal_compressor_pos)

    def _terminal_compressor_neg(m, c):
        return (
            m.terminal_compressor_P_target[c] - m.compressor_P[c, tf]
            <= m.terminal_compressor_abs[c])
    m.terminal_compressor_l1_neg = pyo.Constraint(
        m.Stations, rule=_terminal_compressor_neg)

    terminal_l1_penalty = penalty * (
        sum(m.terminal_pressure_abs[p, vol]
            for p, vol in m.Pipes_VolExtrR_interm)
        + sum(m.terminal_compressor_abs[c] for c in m.Stations)
    )
    m.ObjFun.set_value(m.ObjFun.expr + terminal_l1_penalty)
    return m
...
if __name__ == "__main__":
    result_output_path = os.path.join(_BASE_DIR, "optimal_css_72hrs_gaslib40_soft.xlsx")
