...
def plot_graph_with_layout(G, node_labels=False, edge_labels = False):
    ...
    plt.legend(handles=node_legend_items, loc='lower left',
               bbox_to_anchor=(0.01, 0.1), bbox_transform=plt.gcf().transFigure,
               framealpha=0.9)
    plt.axis("off")
...
if __name__ == "__main__":
    ...
    network_data_path = _os.path.join(_base, 'data', 'data_files', 'Gaslib_40', 'networkData.xlsx')
    input_data_path = _os.path.join(_base, 'data', 'data_files', 'Gaslib_40', 'inputData.xlsx')
...
def write_data_to_excel(sim_data, m_plant, sheets_keys_dict, file_name, terminal_pressure_slack_dict = None, terminal_flow_slack_dict = None,
    ...
    if iteration_times is not None or total_runtime is not None:
        ...
        if iteration_times is not None:
            timing_df['iteration'] = list(iteration_times.keys())
            timing_df['time_s'] = list(iteration_times.values())
        if total_runtime is not None:
            timing_df['total_runtime_s'] = [total_runtime] + [None] * (len(timing_df) - 1)
