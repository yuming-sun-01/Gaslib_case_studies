...
def STATIONS_vars(m):
    ...
    m.compressor_beta = pyo.Var(m.Stations, m.Times, bounds=(1.05, 2), within=pyo.NonNegativeReals, initialize=1.05)
...
# Use collocation <=> add DerivativeVar on pipe_rho; dae.collocation applied later
def PIPE_mass_constr(
        m, scale, dt, method='BACKWARD', dynamic=True):

    assert method in ['BACKWARD', 'FORWARD', 'COLLOCATION'], \
        f"Unknown finite_diff_time method: {method}"

    # =============== collocation mass balance =========================
    if method == 'COLLOCATION' and dynamic:
        # DerivativeVar on pipe_rho: d(pipe_rho[p,vol,t])/dt
        m.drho_dt = DerivativeVar(m.pipe_rho, wrt=m.Times)

        def PIPE_mass_balance_coll_rule(m, p, vol, t):
            N      = m.Nvol[p]
            V_cell = m.Area[p] * m.Length[p] / (N - 1)
            w      = Pipe_MassFlow(m, p, vol,     t, N)
            w_next = Pipe_MassFlow(m, p, vol + 1, t, N)
            cons   = m.wCons[p, vol, t]
            # Density is evaluated on the staggered grid, matching Pipe_DerMass.
            return (m.drho_dt[p, vol + 1, t] * V_cell
                    == (w * scale['w'] - w_next * scale['w'] - cons * scale['w']))

        m.PIPE_mass_balance = pyo.Constraint(
            m.Pipes_VolCenterC, m.Times,
            rule=PIPE_mass_balance_coll_rule)
        return m

    # =============== dynamic mass balance (BACKWARD / FORWARD) =========================
    def PIPE_mass_balance_dynamic_rule(
        ...
        if t == m.Times.first():
            ...
            dMdt = Pipe_DerMass(m, p, vol, t, N, V, scale, dt)
            # RHS: evaluate at current (BACKWARD) or previous (FORWARD) time
            t_RHS = m.Times.prev(t) if method == "FORWARD" else t
            # RHS: mass flow rates
            w      = Pipe_MassFlow(m, p, vol,     t_RHS, N)
            w_next = Pipe_MassFlow(m, p, vol + 1, t_RHS, N)
...
'''
def _wrap_input_time(t_data, t):
    """Return the key in t_data that corresponds to time t, wrapping modulo
    the data period when t exceeds the available range (e.g. t>24 for 24-h data)."""
    if t in t_data:
        return t
    t_keys = sorted(t_data.keys())
    t_min, t_max = t_keys[0], t_keys[-1]
    period = t_max - t_min
    if period <= 0:
        return t_min
    t_eff = (t - t_min) % period + t_min
    if t_eff in t_data:
        return t_eff
    # keys may be int while t_eff is float — round to nearest key
    t_int = int(round(t_eff))
    if t_int in t_data:
        return t_int
    return min(t_keys, key=lambda x: abs(x - t_eff))
...
def fix_exogenous_inputs(m, scale, Options, networkData, inputData):
    ...
    for k in ['pSource', 'wSource', 'wCons']:
        ...
        for indx in indices:
            ...
            if len(indx) == 2:
                t_data = inputData[k][compo0]
                value = t_data[_wrap_input_time(t_data, t)]
            ...
            # e.g. inputData['wCons']['pipe_1'][2][0] = 0.5 kg/s
                ...
                t_data = inputData[k][compo0][compo1]
                value = t_data[_wrap_input_time(t_data, t)]
...
def buildNonLinearModel(
    ...
    m = PIPE_mass_constr(
        m, scale, Opt['dt'],
        method=Opt.get('finite_diff_time', 'BACKWARD'), dynamic=Opt['dynamic']
    )
    m = PIPE_momentum_constr(m, scale, networkData)
    m = PIPE_nlp_auxiliary_constr(m, scale)
    m = PIPE_flow_reversal_constr(m, scale)

    # Apply LAGRANGE-RADAU collocation for finite block when method=COLLOCATION
    if Opt.get('finite_diff_time') == 'COLLOCATION' and Opt.get('dynamic', False):
        nfe = int(round((Opt['T'] - Opt['T0']) / (Opt['dt'] / 3600)))
        ncp = Opt.get('ncp', Opt.get('ncp_finite', 3))
        discretizer = pyo.TransformationFactory('dae.collocation')
        discretizer.apply_to(m, wrt=m.Times, nfe=nfe, ncp=ncp,
                             scheme='LAGRANGE-RADAU')
        # Reduce compressor_P to piecewise-constant (one value per finite element)
        discretizer.reduce_collocation_points(
            m, var=m.compressor_P, ncp=1, contset=m.Times)
        # Re-initialise exogenous inputs at the new collocation interior time
        # points (pSource, wSource, wCons) — fix_exogenous_inputs was called
        # before discretisation and did not cover the interior points.
        m = fix_exogenous_inputs(m, scale, Opt, networkData, inputData)

    # ── NMPC interface sets ────────────────────────────────────────────────────
    # MV: compressor_beta (one per station).  compressor_P is algebraically
    #     derived from beta via STATION_power_balance and appears in ObjFun.
    # CV: node_p at every non-source node.
    # These mirror m.MV_index / m.CV_index in model_equations.py so that
    # make_model_gasnet.py and controller_factory_gasnet.py can use them
    # without special-casing the gas-network model.
    m.MV_index = pyo.Set(
        initialize=list(networkData['Stations'].keys()),
        doc="Manipulated variables: compression ratio beta per station"
    )
    m.CV_index = pyo.Set(
        initialize=[
            n for n in networkData['Nodes']
            if str(networkData['Nodes'][n].get('source', 'nan')) == 'nan'
        ],
        doc="Controlled variables: node pressure at non-source nodes"
    )
...
if __name__ == "__main__":
    ...
    path = r'D:\MPC\pipeline\gas_net\data'
    
    with open(path + r'\Options.json', 'r') as f:
        Opt = json.load(f)
    
    from utils import import_data_from_excel

    networkData, inputData = import_data_from_excel(
        path + r'\data_files\Gaslib_40\networkData.xlsx',
        path + r'\data_files\Gaslib_40\inputData.xlsx'
    )
